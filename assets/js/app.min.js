...
h()}else h()}),a.call(l)}var j,k=a.dispatch("zoom"),l=a.behavior.zoom(),m=!0,n=!0,o=!0;return i.allowPan=function(a){return arguments.length?(m=a,i):m},i.allowZoom=function(a){return arguments.length?(n=a,i):n},i.trackingLatest=function(a){return arguments.length?(o=a,i):o},i.scale=function(a){return arguments.length?(j=a,i):j},a.rebind(i,k,"on"),i}}(d3,fc,sc),function(a,b,c){"use strict";/* Credit to Chris Price for this optimisation
    ...
    */
c.series.candlestick=function(){var c=b.scale.dateTime(),d=a.scale.linear(),e=b.util.fractionalBarWidth(.75),f=function(a,b){return a.date},g=function(a,b){return c(f(a,b))},h=function(a){return a.low},i=function(a){return a.high},j=function(a,b){return a.close},k=b.svg.candlestick().x(function(a){return c(a.date)}).open(function(a){return d(a.open)}).high(function(a){return d(i(a))}).low(function(a){return d(h(a))}).close(function(a){return d(a.close)}),l=b.util.dataJoin().selector("path.up").element("path").attr("class","up"),m=b.util.dataJoin().selector("path.down").element("path").attr("class","down"),n=function(a){a.each(function(a){k.width(e(a.map(g)));var b=a.filter(function(a){return a.open<a.close}),c=a.filter(function(a){return a.open>=a.close});l(this,[b]).attr("d",k),m(this,[c]).attr("d",k)})};return n.xScale=function(a){return arguments.length?(c=a,n):c},n.yScale=function(a){return arguments.length?(d=a,n):d},n.xValue=function(a){return arguments.length?(f=a,n):f},n.yLowValue=function(a){return arguments.length?(h=a,n):h},n.yHighValue=function(a){return arguments.length?(i=a,n):i},n.yCloseValue=function(a){return arguments.length?(j=a,n):j},n.width=function(a){return e(a.map(g))},n}}(d3,fc,sc),function(a,b,c){"use strict";c.app=function(){function d(){c.data.feed.coinbase.products(G,F,E,D,e)}function e(a){
// Add the newly received products to the product list
O.products=O.products.concat(a),C.app.select(".head-menu").datum(O).call(w)}function f(){S&&C.suspendLayout(!1),C.primary.datum(I).call(Q.primary),C.legend.datum(P).call(Q.legend),C.secondaries.datum(J).filter(function(a,b){return b<Q.secondaries.length}).each(function(b,c){a.select(this).attr("class","secondary-container "+Q.secondaries[c].valueString).call(Q.secondaries[c].option)}),C.xAxis.datum(L).call(Q.xAxis),C.navbar.datum(M).call(Q.navbar),C.app.select("#navbar-reset").datum(N).call(x),C.app.select(".head-menu").datum(O).call(w),C.app.select("#selectors").datum(K).call(y),S&&(C.suspendLayout(!0),S=!1)}function g(){S=!0,c.util.layout(C,Q)}function h(){a.select(window).on("resize",function(){g(),R()})}function i(a){var b=[a[0],a[1]];I.viewDomain=b,J.viewDomain=b,L.viewDomain=b,M.viewDomain=b;var d=c.util.domain.trackingLatestData(I.viewDomain,I.data);I.trackingLatest=d,J.trackingLatest=d,M.trackingLatest=d,N.trackingLatest=d,R()}function j(a){P.data=a,R()}function k(){var a=I.data,d=b.util.extent().fields("date")(a),e=c.util.domain.moveToLatest(d,a,.2);i(e)}function l(a){A.select("#loading-message").classed("hidden",!a),A.select("#charts").classed("hidden",a)}function m(a){I.data=a,J.data=a,M.data=a}function n(a){O.selectedProduct=a,I.product=a,J.product=a,P.product=a}function o(a){O.selectedPeriod=a,L.period=a,P.period=a}function p(){return c.chart.primary().on(c.event.crosshairChange,j).on(c.event.viewChange,i)}function q(){return c.chart.nav().on(c.event.viewChange,i)}function r(){return c.menu.navigationReset().on(c.event.resetToLatest,k)}function s(){return c.data.dataInterface().on(c.event.messageReceived,function(a,b){if("error"===a.type||"close"===a.type&&1e3!==a.code)console.log("Error loading data from coinbase websocket: "+a.type+" "+a.code);else if("message"===a.type&&(m(b),I.trackingLatest)){var d=c.util.domain.moveToLatest(I.viewDomain,I.data);i(d)}}).on(c.event.dataLoaded,function(a,b){l(!1),a?console.log("Error getting historic data: "+a):(m(b),P.data=null,k(),g())})}function t(a){return c.menu.head().on(c.event.dataProductChange,function(b){l(!0),n(b.option),o(b.option.periods[0]),"bitcoin"===b.option.family?(a.setNewProduct(b.option.display),a(b.option.periods[0].seconds)):"generated"===b.option.family&&a.generateDailyData(),R()}).on(c.event.dataPeriodChange,function(b){l(!0),o(b.option),a(b.option.seconds),R()})}function u(a,b){b.forEach(function(a){a.isSelected=!1}),a.isSelected=!0}function v(){return c.menu.selectors().on(c.event.primaryChartSeriesChange,function(a){I.series=a,u(a,K.seriesSelector.options),R()}).on(c.event.primaryChartIndicatorChange,function(a){a.isSelected=!a.isSelected,I.indicators=K.indicatorSelector.indicatorOptions.filter(function(a){return a.isSelected}),R()}).on(c.event.secondaryChartChange,function(a){a.isSelected=!a.isSelected,Q.secondaries=K.indicatorSelector.secondaryChartOptions.filter(function(a){return a.isSelected}),
// TODO: This doesn't seem to be a concern of menu.
Q.secondaries.forEach(function(a){a.option.on(c.event.viewChange,i)}),
// TODO: Remove .remove! (could a secondary chart group component manage this?).
C.secondaries.selectAll("*").remove(),g(),R()})}var w,x,y,z={},A=a.select("#app-container"),B=A.select("#charts-container"),C={app:A,charts:B,primary:B.select("#primary-container"),secondaries:B.selectAll(".secondary-container"),xAxis:B.select("#x-axis-container"),navbar:B.select("#navbar-container"),legend:A.select("#legend"),suspendLayout:function(a){var b=this;Object.keys(b).forEach(function(c){"function"!=typeof b[c]&&b[c].layoutSuspended(a)})}},D=c.model.period({display:"Daily",seconds:86400,d3TimeInterval:{unit:a.time.day,value:1},timeFormat:"%b %d"}),E=c.model.period({display:"1 Hr",seconds:3600,d3TimeInterval:{unit:a.time.hour,value:1},timeFormat:"%b %d %Hh"}),F=c.model.period({display:"5 Min",seconds:300,d3TimeInterval:{unit:a.time.minute,value:5},timeFormat:"%H:%M"}),G=c.model.period({display:"1 Min",seconds:60,d3TimeInterval:{unit:a.time.minute,value:1},timeFormat:"%H:%M"}),H=c.model.product({family:"generated",display:"Data Generator",volumeFormat:".3s",periods:[D]}),I=c.model.primaryChart(H),J=c.model.secondaryChart(H),K=c.model.menu.selectors(),L=c.model.xAxis(D),M=c.model.nav(),N=c.model.navigationReset(),O=c.model.headMenu([H],H,D),P=c.model.legend(H,D),Q={primary:void 0,secondaries:[],xAxis:c.chart.xAxis(),navbar:void 0,legend:c.chart.legend()},R=b.util.render(f),S=!0;return z.run=function(){Q.primary=p(),Q.navbar=q();var a=s();w=t(a),x=r(),y=v(),h(),a.generateDailyData(),d()},z}}(d3,fc,sc),function(a,b,c){"use strict";c.app().run()}(d3,fc,sc);