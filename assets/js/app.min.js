...
j=j.filter(function(){return this.parentNode===g.node()}));var k=j.data(i,f),l=k.enter().insert(d).attr(e).style("opacity",mc),m=a.transition(k.exit()).style("opacity",mc).remove();
...
nb(this)})}function qb(){return++Jc}function rb(a,b){function c(b){return a.secondaries.filter(function(a,c){return c===b})}for(var d=0,e=0;e<b.secondaries.length;e++)b.secondaries[e]&&d++;a.secondaries.filter(function(a,b){return d>b}).style("flex","1"),a.secondaries.filter(function(a,b){return b>=d}).style("flex","0"),a.overlaySecondaries.filter(function(a,b){return d>b}).style("flex","1"),a.overlaySecondaries.filter(function(a,b){return b>=d}).style("flex","0");var f=parseInt(a.app.select(".head-row").style("height"),10);Kc||(f+=parseInt(a.app.select(".head-row").style("padding-top"),10)+parseInt(a.app.select(".head-row").style("padding-bottom"),10)+parseInt(a.app.select(".head-row").style("margin-bottom"),10),Kc=!0);var g=window.innerHeight-f;a.charts.style("height",g+"px"),b.xAxis.dimensionChanged(a.xAxis),b.navbar.dimensionChanged(a.navbar),b.primary.dimensionChanged(a.primary);for(var h=0;h<b.secondaries.length;h++)b.secondaries[h].option.dimensionChanged(c(h))}function sb(b,c){var d=a.max(b,function(a){return a.getTime()}),e=a.max(c,function(a){return a.date.getTime()});return d===e}function tb(a,b){var c=Array.isArray(b)?b:[b,b],d=a[1]-a[0];return[a[0]-d*c[0],a[1]+d*c[1]]}function ub(b,c,d){arguments.length<3&&(d=1);var e=Ic.util.extent().fields("date")(c),f=(e[1].getTime()-e[0].getTime())/1e3,g=b.map(function(a){return a.getTime()}),h=d*(a.max(g)-a.min(g))/1e3,i=f>h?[a.time.second.offset(e[1],-h),e[1]]:e;return i}function vb(b,c){var d=a.min(b,function(a){return a.getTime()}),e=a.max(b,function(a){return a.getTime()}),f=c.sort(function(a,b){return a.date-b.date}),g=a.bisector(function(a){return a.date}),h=c.slice(
// Pad and clamp the bisector values to ensure extents can be calculated
Math.max(0,g.left(f,d)-1),Math.min(g.right(f,e)+1,f.length));return h}function wb(b,c,d){var e=Ic.util.extent().fields("date")(c),f=b.map(function(a){return a.getTime()}),g=(a.max(f)-a.min(f))/1e3;if(d.getTime()<e[0]||d.getTime()>e[1])return[new Date(a.min(f)),new Date(a.max(f))];var h=[a.time.second.offset(d,-g/2),a.time.second.offset(d,g/2)],i=0;return h[1].getTime()>e[1].getTime()?i=(e[1].getTime()-h[1].getTime())/1e3:h[0].getTime()<e[0].getTime()&&(i=(e[0].getTime()-h[0].getTime())/1e3),[a.time.second.offset(h[0],i),a.time.second.offset(h[1],i)]}function xb(b){function c(a){
// Don't pan off sides
// Don't pan off sides
return a[0]>=0?-a[0]:a[1]<=0?-a[1]:0}function d(a){
// If zooming, and about to pan off screen, do nothing
return a[0]>0&&a[1]<0}function e(a){var b=j.translate()[0];b+=a,j.translate([b,0])}function f(){j.translate([0,0]),j.scale(1)}function g(a){var g=Ic.util.extent().fields("date")(a.datum().data);j.x(h).on("zoom",function(){var n=h(g[0]),o=h(g[1]),p=d([n,o-b]),q=c([n,o-b]);e(q);var r=1===j.scale(),s=1!==j.scale();if(r&&k||s&&l){var t=h.domain();p?t=g:s&&m&&(t=Mc.domain.moveToLatest(t,a.datum().data)),t[0].getTime()!==t[1].getTime()?i.zoom(t):
// Ensure the user can't zoom-in infinitely, causing the chart to fail to render
// #168, #411
f()}else f()}),a.call(j)}var h,i=a.dispatch("zoom"),j=a.behavior.zoom(),k=!0,l=!0,m=!0;return g.allowPan=function(a){return arguments.length?(k=a,g):k},g.allowZoom=function(a){return arguments.length?(l=a,g):l},g.trackingLatest=function(a){return arguments.length?(m=a,g):m},g.scale=function(a){return arguments.length?(h=a,g):h},a.rebind(g,i,"on"),g}function yb(){function b(b){b.each(function(f){var h=a.select(this).call(j),i=xb(c).scale(e).trackingLatest(g).on("zoom",function(a){d[Nc.viewChange](a)});h.select(".plot-area-container").datum({data:b.datum()}).call(i)})}var c,d=a.dispatch(Nc.viewChange),e=Ic.scale.dateTime(),f=a.scale.linear(),g=!0,h=60,i=Ic.series.multi(),j=Ic.chart.cartesian(e,f).plotArea(i).xTicks(0).yOrient("right").margin({top:0,left:0,bottom:0,right:h});return b.trackingLatest=function(a){return arguments.length?(g=a,b):g},a.rebind(b,d,"on"),a.rebind(b,i,"series","mapping","decorate"),a.rebind(b,j,"yTickValues","yTickFormat","yTicks","xDomain","yDomain"),b.dimensionChanged=function(a){c=parseInt(a.style("width"),10)-h},b}function zb(){function b(a){a.each(function(b){var c=Ic.util.extent().fields("volume").pad(.08)(b.data);c[0]<0&&(c[0]=0),e.yTickFormat(b.product.volumeFormat).trackingLatest(b.trackingLatest).xDomain(b.viewDomain).yDomain(c),a.datum(b.data).call(e)})}var c=a.dispatch(Nc.viewChange),d=Ic.series.bar().yValue(function(a){return a.volume}),e=yb().series([d]).yTicks(4).on(Nc.viewChange,function(a){c[Nc.viewChange](a)});return a.rebind(b,c,"on"),b.dimensionChanged=function(a){e.dimensionChanged(a)},b}function Ab(){function b(a){var b=a.datum();e(b.data),g.trackingLatest(b.trackingLatest).xDomain(b.viewDomain).yDomain([0,100]),a.datum(b.data).call(g)}var c=a.dispatch(Nc.viewChange),d=Ic.indicator.renderer.relativeStrengthIndex(),e=Ic.indicator.algorithm.relativeStrengthIndex(),f=[d.lowerValue(),50,d.upperValue()],g=yb().series([d]).yTickValues(f).on(Nc.viewChange,function(a){c[Nc.viewChange](a)});return a.rebind(b,c,"on"),b.dimensionChanged=function(a){g.dimensionChanged(a)},b}function Bb(){function b(a){var b=a.datum();f(b.data);var c=Ic.util.extent().fields("macd").symmetricalAbout(0).pad(.08)(b.data.map(function(a){return a.macd}));g.trackingLatest(b.trackingLatest).xDomain(b.viewDomain).yDomain(c),a.datum(b.data).call(g)}var c=a.dispatch(Nc.viewChange),d=Ic.annotation.line().value(0).label(""),e=Ic.indicator.renderer.macd(),f=Ic.indicator.algorithm.macd(),g=yb().series([d,e]).yTicks(5).mapping(function(a){return a===d?[0]:this}).decorate(function(a){a.enter().attr("class",function(a,b){return["multi zero","multi"][b]})}).on(Nc.viewChange,function(a){c[Nc.viewChange](a)});return a.rebind(b,c,"on"),b.dimensionChanged=function(a){g.dimensionChanged(a)},b}function Cb(){function b(a){var b=(d.ticks()[1]-d.ticks()[0])/1e3;b<a.seconds?e.ticks(a.d3TimeInterval.unit,a.d3TimeInterval.value):e.ticks(6)}function c(a){var c=a.datum();d.domain(c.viewDomain),b(c.period),a.call(e)}var d=Ic.scale.dateTime(),e=a.svg.axis().scale(d).orient("bottom");return c.dimensionChanged=function(a){d.range([0,parseInt(a.style("width"),10)])},c}function Db(a,b,c,d){return{displayString:a,// TODO: is 'displayName' better?
valueString:b,// TODO: is this an id?
option:c,// TODO: Ideally, remove.
isSelected:!1,icon:d}}function Eb(){var b=Ic.scale.dateTime(),c=a.scale.linear(),d=Ic.util.fractionalBarWidth(.75),e=function(a,b){return a.date},f=function(a,c){return b(e(a,c))},g=function(a){return a.low},h=function(a){return a.high},i=function(a,b){return a.close},j=Ic.svg.candlestick().x(function(a){return b(a.date)}).open(function(a){return c(a.open)}).high(function(a){return c(h(a))}).low(function(a){return c(g(a))}).close(function(a){return c(a.close)}),k=Ic.util.dataJoin().selector("path.up").element("path").attr("class","up"),l=Ic.util.dataJoin().selector("path.down").element("path").attr("class","down"),m=function(a){a.each(function(a){j.width(d(a.map(f)));var b=a.filter(function(a){return a.open<a.close}),c=a.filter(function(a){return a.open>=a.close});k(this,[b]).attr("d",j),l(this,[c]).attr("d",j)})};return m.xScale=function(a){return arguments.length?(b=a,m):b},m.yScale=function(a){return arguments.length?(c=a,m):c},m.xValue=function(a){return arguments.length?(e=a,m):e},m.yLowValue=function(a){return arguments.length?(g=a,m):g},m.yHighValue=function(a){return arguments.length?(h=a,m):h},m.yCloseValue=function(a){return arguments.length?(i=a,m):i},m.width=function(a){return d(a.map(f))},m}function Fb(a,b){var c=b/2;return[[0,0],[c,-c],[a,-c],[a,c],[c,c],[0,0]]}function Gb(a,b){for(var c=a.ticks.apply(a,[]),d=a.domain(),e=0;e<b.length;e++)b[e]>d[0]&&b[e]<d[1]&&c.push(b[e]);return c}function Hb(b,c,d){var e;switch(c.valueString){case"candlestick":case"ohlc":e=[c.option.yLowValue(),c.option.yHighValue()];break;case"line":case"point":e=c.option.yValue();break;case"area":e=c.option.y1Value();break;default:throw new Error("Main series given to chart does not have expected interface")}var f=Ic.util.extent().fields(e)(b);if(d.length){var g=d.map(function(a){return a.valueString}),h=-1!==g.indexOf("movingAverage"),i=-1!==g.indexOf("bollinger");if(i){var j=b.map(function(a){return a.bollingerBands}),k=Ic.util.extent().fields(["lower","upper"])(j);f[0]=a.min([k[0],f[0]]),f[1]=a.max([k[1],f[1]])}if(h){var l=Ic.util.extent().fields("movingAverage")(b);f[0]=a.min([l[0],f[0]]),f[1]=a.max([l[1],f[1]])}if(!h&&!i)throw new Error("Unexpected indicator type")}return f}function Ib(){function b(){var a=[q,i.option,r],b=n.map(function(a){return a.option});return a.concat(b,p)}function c(){switch(w.value(m),x.value(m),r.value(m),i.valueString){case"line":case"point":case"area":i.option.yValue(m)}}
// Call when what to display on the chart is modified (ie series, options)
function d(a){i=a.series,m=a.yValueAccessor.option,n=a.indicators,c(),s.series(b()),v.yTickFormat(a.product.priceFormat),a.selectorsChanged=!1}function e(a){var b=i.option.width(a);p.decorate(function(a){a.classed("band hidden-xs hidden-sm",!0),a.selectAll(".vertical > line").style("stroke-width",b)})}function f(a){a.classed("band",!1).classed("hidden-xs hidden-sm",!0).selectAll("line").style("stroke-width",null)}function g(a){"candlestick"===i.valueString||"ohlc"===i.valueString?e(a):p.decorate(f)}function h(b){var c=b.datum();c.selectorsChanged&&d(c),v.xDomain(c.viewDomain),p.snap(Ic.util.seriesPointSnapXOnly(i.option,c.data)),g(c.data),w(c.data),x(c.data);
// Scale y axis
var e=Mc.domain.filterDataInDateRange(v.xDomain(),c.data),f=Hb(e,i,n),h=Mc.domain.padYDomain(f,.04);v.yDomain(h);
...
// Redraw
v.plotArea(s),b.call(v);var r=xb(j).scale(t).trackingLatest(c.trackingLatest).on("zoom",function(a){l[Nc.viewChange](a)});b.select(".plot-area").call(r)}var i,j,k=60,l=a.dispatch(Nc.viewChange,Nc.crosshairChange),m=function(a){return a.close},n=[],o=[],p=Ic.tool.crosshair().xLabel("").yLabel("").on("trackingmove",function(a){a.length>0?l.crosshairChange(a[0].datum):l.crosshairChange(void 0)}).on("trackingend",function(){l.crosshairChange(void 0)});p.id=Mc.uid();var q=Ic.annotation.gridline().yTicks(5).xTicks(0),r=Ic.annotation.line().orient("horizontal").value(m).label("");r.id=Mc.uid();var s=Ic.series.multi().key(function(a){return a.id}).mapping(function(a){switch(a){case r:return[this.data[this.data.length-1]];case p:return o;default:return this.data}}),t=Ic.scale.dateTime(),u=a.scale.linear(),v=Ic.chart.cartesian(t,u).xTicks(0).yOrient("right").margin({top:0,left:0,bottom:0,right:k}),w=Ic.indicator.algorithm.movingAverage(),x=Ic.indicator.algorithm.bollingerBands();
// Call when the main layout is modified
return a.rebind(h,l,"on"),h.dimensionChanged=function(a){j=parseInt(a.style("width"),10)-k},h}function Jb(){function b(a,b){a.select(".plot-area").selectAll(".e, .w").classed("hidden",b)}function c(a){return a.extent()[0][0]-a.extent()[1][0]===0}function d(a){var d=a.datum();q.domain(d.viewDomain);var f=Mc.domain.filterDataInDateRange(Ic.util.extent().fields("date")(d.data),d.data),g=Ic.util.extent().fields(["low","high"])(f);p.xDomain(Ic.util.extent().fields("date")(d.data)).yDomain(g),t.on("brush",function(){var d=c(t);
// Hide the bar if the extent is empty
b(a,d),d||o[Nc.viewChange]([t.extent()[0][0],t.extent()[1][0]])}).on("brushend",function(){var e=c(t);b(a,!1),e&&o[Nc.viewChange](Mc.domain.centerOnDate(q.domain(),d.data,t.extent()[0][0]))}),p.plotArea(u),a.call(p);
// Allow to zoom using mouse, but disable panning
var h=xb(e).scale(q).trackingLatest(d.trackingLatest).allowPan(!1).on("zoom",function(a){o[Nc.viewChange](a)});a.select(".plot-area").call(h)}var e,f=100,g=40,h=f-g,i=2,j=i/2,k=h-j,l=k,m=j+l/2,n=2,o=a.dispatch(Nc.viewChange),p=Ic.chart.cartesian(Ic.scale.dateTime(),a.scale.linear()).yTicks(0).margin({bottom:g}),q=Ic.scale.dateTime(),r=Ic.series.area().yValue(function(a){return a.close}),s=Ic.series.line().yValue(function(a){return a.close}),t=a.svg.brush(),u=Ic.series.multi().series([r,s,t]).decorate(function(a){var b=a.enter();a.select(".extent").attr("height",k).attr("y",i/2),
...
function Vb(a){return{displayString:a.display,option:a}}function Wb(a,b,c){return{productConfig:{title:null,careted:!0,listIcons:!1,icon:!1},mobilePeriodConfig:{title:null,careted:!1,listIcons:!1,icon:!1},products:a,selectedProduct:b,selectedPeriod:c}}function Xb(){var a=Eb();a.id=Mc.uid();var b=Db("Candlestick","candlestick",a,"sc-icon-candlestick-series");b.isSelected=!0;var c=Ic.series.ohlc();c.id=Mc.uid();var d=Ic.series.line();d.id=Mc.uid();var e=Ic.series.point();e.id=Mc.uid();var f=Ic.series.area();return f.id=Mc.uid(),{config:{title:null,careted:!1,listIcons:!0,icon:!0},options:[b,Db("OHLC","ohlc",c,"sc-icon-ohlc-series"),Db("Line","line",d,"sc-icon-line-series"),Db("Point","point",e,"sc-icon-point-series"),Db("Area","area",f,"sc-icon-area-series")]}}function Yb(){return{config:{title:"Add Indicator",careted:!1,listIcons:!0,icon:!1},indicatorOptions:Uc,secondaryChartOptions:Vc}}function Zb(){
// TODO: Instantiate series/indicator components outside of menu model?
return{seriesSelector:Xb(),indicatorSelector:Yb()}}function $b(){function b(b){b.each(function(b){var d=a.select(this),e=d.selectAll("div").data(b.selectedIndicators,function(a){return a.valueString}),f=e.enter().append("div").attr("class","edit-indicator");f.append("span").attr("class","icon sc-icon-delete").on("click",c.indicatorChange),f.append("span").attr("class","indicator-label").text(function(a){return a.displayString}),e.exit().remove()})}var c=a.dispatch(Nc.indicatorChange);return a.rebind(b,c,"on"),b}function _b(){var b=a.dispatch(Nc.primaryChartIndicatorChange,Nc.secondaryChartChange),c=$b().on(Nc.indicatorChange,b[Nc.primaryChartIndicatorChange]),d=$b().on(Nc.indicatorChange,b[Nc.secondaryChartChange]),e=function(b){b.each(function(b){var e=a.select(this);e.select("#overlay-primary-container .edit-indicator-container").datum({selectedIndicators:b.primaryIndicators}).call(c),e.selectAll(".overlay-secondary-container").each(function(c,e){var f=a.select(this),g=b.secondaryIndicators[e]?[b.secondaryIndicators[e]]:[];f.select(".edit-indicator-container").datum({selectedIndicators:g}).call(d)})})};return a.rebind(e,b,"on"),e}function ac(){function b(a){var b=a.datum(),d=a.selectAll("g").data([b]);d.enter().append("g").attr("class","reset-button").on("click",function(){c[Nc.resetToLatest]()}).append("path").attr("d","M1.5 1.5h13.438L23 20.218 14.937 38H1.5l9.406-17.782L1.5 1.5z"),d.classed("active",!b.trackingLatest)}var c=a.dispatch(Nc.resetToLatest);return a.rebind(b,c,"on"),b}function bc(){function b(a){var b=a.datum(),h=b.selectedIndex||0,i=b.config,j=d(a,[b.options]);if(i.icon){var k=j.selectAll(".icon").data([0]);k.enter().append("span"),k.attr("class","icon "+b.options[h].icon)}else j.select(".icon").remove(),j.text(function(){return i.title||b.options[h].displayString});e(j,i.careted?[0]:[]);var l=f(a,[b.options]),m=g(l,b.options),n=m.enter().on("click",c.optionChange).append("a").attr("href","#");n.append("span").attr("class","icon"),n.append("span").attr("class","name"),m.selectAll(".icon").attr("class",function(a){return"icon "+a.icon}),m.selectAll(".name").text(function(a){return a.displayString})}var c=a.dispatch("optionChange"),d=Ic.util.dataJoin().selector("button").element("button").attr({"class":"dropdown-toggle",type:"button","data-toggle":"dropdown"}),e=Ic.util.dataJoin().selector(".caret").element("span").attr("class","caret"),f=Ic.util.dataJoin().selector("ul").element("ul").attr("class","dropdown-menu"),g=Ic.util.dataJoin().selector("li").element("li").key(function(a){return a.displayString});return a.rebind(b,c,"on"),b}function cc(){var b=a.dispatch(Nc.primaryChartSeriesChange,Nc.primaryChartIndicatorChange,Nc.secondaryChartChange),c=bc().on("optionChange",b[Nc.primaryChartSeriesChange]),d=bc().on("optionChange",function(a){0===a.valueString.indexOf("secondary")?b[Nc.secondaryChartChange](a):b[Nc.primaryChartIndicatorChange](a)}),e=function(b){b.each(function(b){var e=a.select(this),f=b.seriesSelector.options.map(function(a){return a.isSelected}).indexOf(!0);e.select(".series-dropdown").datum({config:b.seriesSelector.config,options:b.seriesSelector.options,selectedIndex:f}).call(c);var g=b.indicatorSelector.indicatorOptions.concat(b.indicatorSelector.secondaryChartOptions),h=g.map(function(a,b){return a.isSelected?b:null}).filter(function(a){return a});e.select(".indicator-dropdown").datum({config:b.indicatorSelector.config,options:g,selected:h}).call(d)})};return a.rebind(e,b,"on"),e}function dc(){function b(a){var b=a.datum().selectedIndex||0,e=d(a,[a.datum().options]);e.enter().append("ul");var f=e.selectAll("li").data(Ic.util.fn.identity);f.enter().append("li").append("a").attr("href","#").on("click",c.tabClick),f.classed("active",function(a,c){return c===b}).select("a").text(function(a){return a.displayString})}var c=a.dispatch("tabClick"),d=Ic.util.dataJoin().selector("ul").element("ul");return a.rebind(b,c,"on"),b}function ec(){var b=a.dispatch(Nc.dataProductChange,Nc.dataPeriodChange,Nc.clearAllPrimaryChartIndicatorsAndSecondaryCharts),c=bc().on("optionChange",b[Nc.dataProductChange]),d=dc().on("tabClick",b[Nc.dataPeriodChange]),e=bc().on("optionChange",b[Nc.dataPeriodChange]),f=function(f){f.each(function(g){var h=a.select(this),i=g.products,j={config:g.productConfig,options:i.map(Ub),selectedIndex:i.indexOf(g.selectedProduct)};h.select("#product-dropdown").datum(j).call(c),h.select("#mobile-product-dropdown").datum(j).call(c);var k=g.selectedProduct.periods;h.select("#period-selector").classed("hidden",k.length<=1).datum({options:k.map(Vb),selectedIndex:k.indexOf(g.selectedPeriod)}).call(d),h.select("#mobile-period-selector").classed("hidden",k.length<=1).datum({config:g.mobilePeriodConfig,options:k.map(Vb),selectedIndex:k.indexOf(g.selectedPeriod)}).call(e),h.select("#clear-indicators").on("click",b[Nc.clearAllPrimaryChartIndicatorsAndSecondaryCharts]),f.select("#toggle-button").on("click",function(){b[Nc.toggleSlideout]()})})};return a.rebind(f,b,"on"),f}function fc(){function a(a){var c=++b;return function(d,e){b>c||a(d,e)}}var b=0;return a.invalidateCallback=function(){return b++,a},a}function gc(){function b(a){var b=1e3*f;return new Date(Math.floor(a.getTime()/b)*b)}var c=function(a){return a.date},d=function(a){return Number(a.volume)},e=function(a){return Number(a.price)},f=60,g=function(f,g){var h=b(c(g)),i=e(g),j=d(g),k=a.bisector(function(a){return a.date}).left,l=f.filter(function(a){return a.date.getTime()===h.getTime()})[0];l?(l.high=Math.max(i,l.high),l.low=Math.min(i,l.low),l.close=i,l.volume+=j):f.splice(k(f,h),0,{date:h,open:i,high:i,low:i,close:i,volume:j})};return g.granularity=function(a){return arguments.length?(f=a,g):f},g.price=function(a){return arguments.length?(e=a,g):e},g.volume=function(a){return arguments.length?(d=a,g):d},g.date=function(a){return arguments.length?(c=a,g):c},g}
...
function b(b){var c=new Date,d=a.time.second.offset(c,-l*b);f.start(d).end(c)}function c(a){d(),f.granularity(a),b(a),j.granularity(a),f(g(function(a,b){a?k[Nc.dataLoadError](a):(m=b,k[Nc.dataLoaded](e(m)))})),i.on("message",function(a){j(m,a),k[Nc.newTrade](m)}),i.on("error",function(a){
// TODO: The 'close' event is potentially more useful for error info.
k[Nc.webSocketError](a)}),i()}function d(){i.close(),m=[],g.invalidateCallback()}function e(a){return a.sort(function(a,b){return a.date-b.date})}var f=Ic.data.feed.coinbase(),g=fc(),h=Ic.data.random.financial(),i=hc(),j=gc().date(function(a){return new Date(a.time)}).volume(function(a){return Number(a.size)}),k=a.dispatch(Nc.newTrade,Nc.dataLoaded,Nc.dataLoadError,Nc.webSocketError),l=200,m=[];
// TODO: remove? - #407
return c.generateDailyData=function(){d();var a=new Date;a.setHours(0,0,0,0);var b=864e5;h.startDate(new Date(a-(l-1)*b));var c=h(l);k[Nc.dataLoaded](c)},a.rebind(c,k,"on"),c}function jc(){function b(){X&&F.suspendLayout(!1),F.primary.datum(M).call(V.primary),F.legend.datum(T).call(V.legend),F.secondaries.datum(N).filter(function(a,b){return b<V.secondaries.length}).each(function(b,c){a.select(this).attr("class","secondary-container "+V.secondaries[c].valueString).call(V.secondaries[c].option)}),F.xAxis.datum(P).call(V.xAxis),F.navbar.datum(Q).call(V.navbar),F.app.select("#navbar-reset").datum(R).call(z),F.app.select(".head-menu").datum(S).call(y),F.app.selectAll(".selectors").datum(O).call(A),F.overlay.datum(U).call(E),X&&(F.suspendLayout(!0),X=!1)}function c(){X=!0,Mc.layout(F,V)}function d(){a.select(window).on("resize",function(){c(),W()})}function e(a){var b=[a[0],a[1]];M.viewDomain=b,N.viewDomain=b,P.viewDomain=b,Q.viewDomain=b;var c=Mc.domain.trackingLatestData(M.viewDomain,M.data);M.trackingLatest=c,N.trackingLatest=c,Q.trackingLatest=c,R.trackingLatest=c,W()}function f(a){a.isSelected=!a.isSelected,v(),W()}function g(a){a.isSelected=!a.isSelected,w(),W()}function h(a){T.data=a,W()}function i(){var a=M.data,b=Ic.util.extent().fields("date")(a),c=Mc.domain.moveToLatest(b,a,.2);e(c)}function j(a){C.select("#loading-message").classed("hidden",!a),C.select("#charts").classed("hidden",a)}function k(a){M.data=a,N.data=a,Q.data=a}function l(a){S.selectedProduct=a,M.product=a,N.product=a,T.product=a}function m(a){S.selectedPeriod=a,P.period=a,T.period=a}function n(){return Pc.primary().on(Nc.crosshairChange,h).on(Nc.viewChange,e)}function o(){return Pc.nav().on(Nc.viewChange,e)}function p(){return Yc.navigationReset().on(Nc.resetToLatest,i)}function q(){return ic().on(Nc.newTrade,function(a){if(k(a),M.trackingLatest){var b=Mc.domain.moveToLatest(M.viewDomain,M.data);e(b)}}).on(Nc.dataLoaded,function(a){j(!1),k(a),T.data=null,i(),c()}).on(Nc.dataLoadError,function(a){console.log("Error getting historic data: "+a)}).on(Nc.webSocketError,function(a){console.log("Error loading data from websocket: "+a)})}function r(a){return Yc.head().on(Nc.dataProductChange,function(b){j(!0),l(b.option),m(b.option.periods[0]),b.option===L?a(b.option.periods[0].seconds):b.option===K&&a.generateDailyData(),W()}).on(Nc.dataPeriodChange,function(b){j(!0),m(b.option),a(b.option.seconds),W()}).on(Nc.clearAllPrimaryChartIndicatorsAndSecondaryCharts,function(){M.indicators.forEach(t),V.secondaries.forEach(t),v(),w(),W()})}function s(a,b){b.forEach(function(a){a.isSelected=!1}),a.isSelected=!0}function t(a){a.isSelected=!1}function u(){return Yc.selectors().on(Nc.primaryChartSeriesChange,function(a){M.series=a,s(a,O.seriesSelector.options),W()}).on(Nc.primaryChartIndicatorChange,f).on(Nc.secondaryChartChange,g)}function v(){M.indicators=O.indicatorSelector.indicatorOptions.filter(function(a){return a.isSelected}),U.primaryIndicators=M.indicators}function w(){V.secondaries=O.indicatorSelector.secondaryChartOptions.filter(function(a){return a.isSelected}),
// TODO: This doesn't seem to be a concern of menu.
V.secondaries.forEach(function(a){a.option.on(Nc.viewChange,e)}),U.secondaryIndicators=V.secondaries,
// TODO: Remove .remove! (could a secondary chart group component manage this?).
F.secondaries.selectAll("*").remove(),c()}function x(){return Yc.overlay().on(Nc.primaryChartIndicatorChange,f).on(Nc.secondaryChartChange,g)}var y,z,A,B={},C=a.select("#app-container"),D=C.select("#charts-container"),E=C.select("#overlay"),F={app:C,charts:D,primary:D.select("#primary-container"),secondaries:D.selectAll(".secondary-container"),xAxis:D.select("#x-axis-container"),navbar:D.select("#navbar-container"),overlay:E,overlaySecondaries:E.selectAll(".overlay-secondary-container"),legend:C.select("#legend"),suspendLayout:function(a){var b=this;Object.keys(b).forEach(function(c){"function"!=typeof b[c]&&b[c].layoutSuspended(a)})}},G=Xc.data.period({display:"Daily",seconds:86400,d3TimeInterval:{unit:a.time.day,value:1},timeFormat:"%b %d"}),H=Xc.data.period({display:"1 Hr",seconds:3600,d3TimeInterval:{unit:a.time.hour,value:1},timeFormat:"%b %d %Hh"}),I=Xc.data.period({display:"5 Min",seconds:300,d3TimeInterval:{unit:a.time.minute,value:5},timeFormat:"%H:%M"}),J=Xc.data.period({display:"1 Min",seconds:60,d3TimeInterval:{unit:a.time.minute,value:1},timeFormat:"%H:%M"}),K=Xc.data.product({display:"Data Generator",volumeFormat:".3s",periods:[G]}),L=Xc.data.product({display:"Bitcoin",volumeFormat:".2f",periods:[J,I,H]}),M=Xc.chart.primary(K),N=Xc.chart.secondary(K),O=Xc.menu.selectors(),P=Xc.chart.xAxis(G),Q=Xc.chart.nav(),R=Xc.chart.navigationReset(),S=Xc.menu.head([K,L],K,G),T=Xc.chart.legend(K,G),U=Xc.menu.overlay(),V={primary:void 0,secondaries:[],xAxis:Pc.xAxis(),navbar:void 0,legend:Pc.legend()},W=Ic.util.render(b),X=!0;return B.run=function(){V.primary=n(),V.navbar=o();var a=q();y=r(a),z=p(),A=u(),E=x(),c(),d(),a.generateDailyData()},B}a="default"in a?a["default"]:a;var kc={boundingBox:p,greedy:o,local:n},lc=Object.freeze({context:q,identity:r,index:s,noop:t}),mc=1e-6,nc=Object.freeze({isOrdinal:v,range:w,setRange:x}),oc={rectangles:z,strategy:kc},pc={dataJoin:u,expandRect:J,extent:I,fn:lc,minimum:e,fractionalBarWidth:H,innerDimensions:G,rebind:b,rebindAll:d,scale:nc,noSnap:B,pointSnap:C,seriesPointSnap:D,seriesPointSnapXOnly:E,seriesPointSnapYOnly:F,render:A,arrayFunctor:y},qc={crosshair:R,fibonacciFan:M,container:L,measure:K},rc={axis:W,bar:V,candlestick:U,ohlc:T,errorBar:S},sc={waterfall:X},tc={area:ha,bar:fa,stack:da,line:ea},uc={area:ga,axis:la,bar:Y,candlestick:ka,cycle:ja,line:ca,multi:O,ohlc:ia,point:Q,stacked:tc,groupedBar:ba,xyBase:P,ohlcBase:aa,errorBar:_,waterfall:Z,algorithm:sc};pa.tickTransformer=na;var vc={discontinuity:{identity:ma,skipWeekends:qa},dateTime:pa},wc={bollingerBands:xa,macd:wa,relativeStrengthIndex:va,stochasticOscillator:ua,forceIndex:ta,envelope:sa,elderRay:ra},xc={bollingerBands:Ra,exponentialMovingAverage:ya,macd:Na,percentageChange:Qa,relativeStrengthIndex:Ka,stochasticOscillator:Ia,slidingWindow:Aa,undefinedInputAdapter:Ea,forceIndex:Ga,envelope:Da,elderRay:za},yc={bollingerBands:Sa,calculator:xc,exponentialMovingAverage:Pa,macd:Oa,merge:Ba,movingAverage:Ma,relativeStrengthIndex:La,stochasticOscillator:Ja,forceIndex:Ha,envelope:Fa,elderRay:Ca},zc={algorithm:yc,renderer:wc},Ac={modeMedian:Wa,largestTriangleThreeBucket:Va,largestTriangleOneBucket:Ua,bucket:Ta},Bc={filter:{skipWeekends:$a},financial:Za,walk:Ya},Cc={coinbase:ab,quandl:_a},Dc={feed:Cc,random:Bc,spread:Xa,sampler:Ac};/**
...
sXML="<svg xmlns='http://www.w3.org/2000/svg'>"+a+"</svg>";for(var c=b.parseFromString(sXML,"text/xml").documentElement,d=c.firstChild;d;)this.appendChild(this.ownerDocument.importNode(d,!0)),d=d.nextSibling}catch(e){throw new Error("Error parsing XML string")}}})}}((1,eval)("this").SVGElement);var Ec={cartesian:eb,sparkline:db,tooltip:cb,smallMultiples:bb},Fc={band:gb,gridline:fb,line:N},Gc=function(a){var b=a.exports;
...
var Hc="development",Ic={annotation:Fc,chart:Ec,data:Dc,indicator:zc,scale:vc,series:uc,svg:rc,tool:qc,util:pc,version:Hc,layout:oc},Jc=0,Kc=!1,Lc={centerOnDate:wb,filterDataInDateRange:vb,moveToLatest:ub,padYDomain:tb,trackingLatestData:sb},Mc={domain:Lc,layout:rb,uid:qb},Nc={crosshairChange:"crosshairChange",viewChange:"viewChange",newTrade:"newTrade",dataLoaded:"dataLoaded",dataLoadError:"dataLoadError",webSocketError:"webSocketError",dataProductChange:"dataProductChange",dataPeriodChange:"dataPeriodChange",resetToLatest:"resetToLatest",clearAllPrimaryChartIndicatorsAndSecondaryCharts:"clearAllPrimaryChartIndicatorsAndSecondaryCharts",primaryChartSeriesChange:"primaryChartSeriesChange",primaryChartYValueAccessorChange:"primaryChartYValueAccessorChange",primaryChartIndicatorChange:"primaryChartIndicatorChange",secondaryChartChange:"secondaryChartChange",indicatorChange:"indicatorChange"},Oc={base:yb,macd:Bb,rsi:Ab,volume:zb},Pc={legend:Kb,nav:Jb,primary:Ib,xAxis:Cb,secondary:Oc},Qc={period:Mb,product:Lb},Rc={legend:Sb,nav:Rb,navigationReset:Qb,primary:Pb,secondary:Ob,xAxis:Nb},Sc=Ic.series.line().decorate(function(a){a.enter().classed("movingAverage",!0)}).yValue(function(a){return a.movingAverage});Sc.id=Mc.uid();var Tc=Ic.indicator.renderer.bollingerBands();Tc.id=Mc.uid();var Uc=[Db("Moving Average","movingAverage",Sc,"sc-icon-moving-average-indicator"),Db("Bollinger Bands","bollinger",Tc,"sc-icon-bollinger-bands-indicator")],Vc=[Db("Relative Strength Index","secondary-rsi",Oc.rsi(),"sc-icon-rsi-indicator"),Db("MACD","secondary-macd",Oc.macd(),"sc-icon-macd-indicator"),Db("Volume","secondary-volume",Oc.volume(),"sc-icon-bar-series")],Wc={selectors:Zb,head:Wb,periodAdaptor:Vb,productAdaptor:Ub,overlay:Tb},Xc={menu:Wc,chart:Rc,data:Qc},Yc={head:ec,selectors:cc,navigationReset:ac,overlay:_b};jc().run()});